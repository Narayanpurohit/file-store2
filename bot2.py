...
async def handle_start(client, message: Message):
    ...
    if slug.startswith("fs_"):
        ...
        asyncio.create_task(delete_message_after_delay(client, message.chat.id, sent.id, 30))
...
async def admin_upgrade_user(client, message: Message):
    ...
    await client.send_message(user_id, f"You have been upgraded for **{days} day(s)**.")
...
async def check_verification(client, message: Message):
    ...
    return await message.reply(f"You are verified for another {hours}h {minutes}m.")
